...
from ip2geotools.databases.commercial import (
    DbIpWeb,
    MaxMindGeoIp2City,
    Ip2LocationWeb,
    NeustarWeb,
    ...
    def execute(self):
        ...
        parser = argparse.ArgumentParser(
            ...
            epilog=(
                ...
                + "for master's thesis at FEEC BUT 2017/2018"
            ).format(
        ...
        parser.add_argument("IP_ADDRESS", help="IP address to be checked")
        ...
        parser.add_argument(
            "--api_key", help="API key for given geolocation database (if needed)", dest="api_key"
        )

        parser.add_argument(
            "--db_path", help="path to geolocation database file (if needed)", dest="db_path"
        )
        ...
        parser.add_argument(
            ...
            default="inline",
            type=str.lower,
            choices=["json", "xml", "csv-space", "csv-tab", "inline"],
        )
        ...
        try:
            ...
            if arguments.database == "dbipcity":
                ip_location = DbIpCity.get(arguments.IP_ADDRESS, api_key=arguments.api_key)
                ...
                ip_location = MaxMindGeoLite2City.get(
                    arguments.IP_ADDRESS, db_path=arguments.db_path
                )
            elif arguments.database == "ip2location":
                ip_location = Ip2Location.get(arguments.IP_ADDRESS, db_path=arguments.db_path)
            ...
            # commercial databases
                ...
                ip_location = SkyhookContextAcceleratorIp.get(
                    arguments.IP_ADDRESS, username=arguments.username, password=arguments.password
                )
            elif arguments.database == "ipinfo":
                ip_location = IpInfo.get(arguments.IP_ADDRESS)
            elif arguments.database == "eurek":
                ip_location = Eurek.get(arguments.IP_ADDRESS, api_key=arguments.api_key)
